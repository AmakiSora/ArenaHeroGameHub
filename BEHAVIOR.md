...
- 任何可见敌人所在的格 → 自动加入踪迹，并记录该格**最后看到的兵种类型**
  （总/工/先/游）；同一格出现多个敌人时按 **CORE > 先锋/游侠 > 工人 > 未知**
  的优先级保留类型——敌方核心脚下常站着它刚生产出的工人，工人的目击不能把
  总部标签降级成工人
...
路径只绘制**尚未走过的剩余段**（以单位当前所在格为起点截断），单位沿路径推进时线条随之缩短，
已走过的地方不再残留路径。
...
localStorage，跨页面刷新与软刷新保留。**同格去重：** 同一格出现多个敌人时
（敌人核心脚下站着工人），只画优先级最高的一个标记，避免蓝色工人把敌人总部盖住。
...
| **自动进攻** | 记忆中离 core 与小队都近的敌人 | 朝"距核心距离 + 距进攻队当前重心距离"最小的目击点狩猎，全队同目标 |
...
Core 按仪表盘配置的**按兵种需求目标**创建单位：当前数量低于目标就生产，达到目标就停止，超出目标也停止生产且保留现有单位（不再自裁）；兵阵亡后自动补到目标。
...
- 当前数量 < 目标 → 自动生产该兵种（基础价：工人 5，先锋 10，游侠 12 资源；人口 ≥ 20 后单价随人口上浮，第 21 个起每次 +30%，每 5 个再涨一档），每 Tick 生产 1 个
- 当前数量 == 目标 → 停止生产
- 当前数量 > 目标 → 停止生产，不对现有单位做任何处理（超过上限不再自裁）
...
- **被击败**：我方单位被击败 / 超编自裁 / 核心受击或被摧毁
- **战斗**：游侠击中、先锋横扫命中、我方单位承伤（默认隐藏，可开启）
- **经济**：挖矿 / 卸货 / 信标加成 / 修盾回血 / 生产（默认隐藏）
