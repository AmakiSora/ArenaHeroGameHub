...
**自动进攻遇敌撤退**（`attack_retreat_radius`，默认 5 格，0=关闭）：**仅进攻队 + 自动进攻模式生效**（坐标/信标模式与守家队/游击队均不受此配置影响）。每 Tick 以进攻队重心为中心，统计半径（切比雪夫距离）内**敌方战斗单位**（先锋 / 游侠 / 核心，不含工人）数量；若 ≥ 本队人数，则全队**撤离敌群质心方向**，并把该敌群覆盖的记忆目击点加入临时禁忌集——本 Tick 自动目标评分跳过这些点，转向次优目击另寻目标（若全部目击点都属于该敌群则回退至进攻坐标重组）。撤退与另寻目标均在本 Tick 立即生效。
...
计数只统计**敌方战斗单位**（先锋 / 游侠 / 核心，不含工人）：敌方核心格上的工人护卫不会把单挑判成 ≥3 而撤退，落单的核心可被捡走。工人同样不参与撤退质心和单挑目标选择——游击队只追/打/扫威胁，路过工人不纠缠。
