...
**自动进攻遇敌撤退**（`attack_retreat_radius`，默认 5 格，0=关闭）：仅自动进攻生效。每 Tick 以进攻队重心为中心，统计半径（切比雪夫距离）内**敌方战斗单位**（先锋 / 游侠 / 核心，不含工人）数量；若 ≥ 本队人数，则全队**撤离敌群质心方向**，并把该敌群覆盖的记忆目击点加入临时禁忌集——本 Tick 自动目标评分跳过这些点，转向次优目击另寻目标（若全部目击点都属于该敌群则回退至进攻坐标重组）。撤退与另寻目标均在本 Tick 立即生效。
