...
- 任何可见敌人所在的格 → 自动加入踪迹，并记录该格**最后看到的兵种类型**
  （★/工/先/游，其中 ★ 表示敌方总部）；同一格出现多个敌人时按 **CORE > 先锋/游侠 > 工人 > 未知**
...
**容量感知移动（防 CELL_UNIT_LIMIT 撞墙）：**
- 服务器每格最多叠 2 个单位；战斗单位的移动规划（守家巡逻/回防/回血返程/进攻行军/航点接敌）会感知本 Tick 各格友军数量，已满格按墙处理，自动换轴绕行或原地等待
- 工人的 `MOVE_CONTESTED` 重试退避：同一位置朝同一目标格连续 2 Tick 被判抢格失败（两工人确定性计划顶牛，双双取消）→ 停 1 Tick 让对方先走，随后恢复正常规划
...
先锋与游侠通过策略配置编入四支队伍。名单使用仪表盘上的稳定单位名
（`V1`、`R2` 等），逗号/空格/分号分隔。同一单位出现在多队时优先级为：
**守家 > 进攻 > 风筝 > 游击**。

**自动入队：** 新生产出来的先锋/游侠（以及任何仍未编队的战斗单位）会在下一个 Tick
自动写入 `home_team` 并热保存到 `tactic_config.json`。已在进攻队、风筝队或游击队中的单位不会被改派。
仪表盘刷新后可看到名单更新；也可手动把名字改到进攻/风筝/游击队。
...
| 进攻方式 | 目标 | 说明 |
|----------|------|------|
| **进攻坐标** | `attack_target_x/y` | 集体朝配置坐标推进，只接战沿途敌人 |
...
**坐标/信标沿途接敌**（`attack_march_engage_radius`，默认 0，范围 0–500）：坐标与信标模式都是"朝目标推进，沿途接战"，因此**只追顺路敌人**——到目标点的距离不超过本单位当前到目标点的距离（敌人在目标侧/前进方向）。反方向、偏离行军方向的可见敌人一律忽略，队伍继续朝目标行军；已到达目标点时只打贴身/射程内敌人，原地待命不再被远处敌人带离坐标。N>0 时额外限定只追**距本单位 N 格内**的顺路敌人，防止为远处敌人大幅绕路；0 = 仅顺路过滤。自动进攻模式不受此配置影响（auto 用 `attack_auto_radius`）。

**自动进攻遇敌撤退**（`attack_retreat_radius`，默认 5 格，0=关闭）：**仅进攻队 + 自动进攻模式生效**（坐标/信标模式与守家队/游击队均不受此配置影响）。每 Tick 以进攻队重心为中心，统计半径（切比雪夫距离）内**敌方攻击单位**（先锋 / 游侠，不含工人和无攻击能力的核心）数量；若 ≥ 本队人数，则全队**撤离敌群质心方向**，并把该敌群覆盖的记忆目击点加入临时禁忌集——本 Tick 自动目标评分跳过这些点，转向次优目击另寻目标（若全部目击点都属于该敌群则回退至进攻坐标重组）。撤退与另寻目标均在本 Tick 立即生效。

**自动进攻半径**（`attack_auto_radius`，默认 100，范围 0–500，0=不限制）：仅自动进攻（auto 模式）生效，坐标/信标模式不受影响。值 N>0 时，自动目标候选与沿途接敌均被限制在距核心 N 格以内（曼哈顿距离）：目标评分只考虑半径内的目击点（半径内无候选时回退至静态进攻坐标），沿途只追击半径内的可见敌人（半径外敌人直接忽略）。该半径**不约束撤退分支**（`attack_retreat_radius` 遇敌撤退不受影响）。0 表示完全不限制，与旧版行为完全一致。

**行军受阻自动返家集结**：进攻行军寻路失败（目标超出 A* 扩展预算，或目标向相邻格被墙挡死）时，单位自动切守家队返家逻辑——用**全向贪心步进**（四方向按到核心曼哈顿距离取最优，仍过滤墙/死胡同/满格，不依赖寻路预算；并带**防回头**记忆，两格窄缝不会来回弹）走向核心。返家是**粘性**的：一旦触发就持续走回家、不再每 Tick 重试进攻行军（避免两个规划器在墙角把单位来回掰），走到核心格后返家状态清除、由被动回血恢复 HP，之后目标重新进入寻路预算，进攻自动恢复，无需人工改队伍名册。

| 优先级 | 条件 | 动作 | 说明 |
|--------|------|------|------|
| 0 | 自动进攻 + 敌方战斗单位 ≥ 本队 | **MOVE** 远离敌群质心 | 撤退另寻目标，不硬刚 |
| 1 | 可立即攻击 | **SWEEP** / **SHOOT** | 中途主动出击 |
| 2 | 有顺路敌人 | **MOVE** → 最近顺路敌人 | 坐标/信标只追目标侧敌人，反方向忽略；auto 全追 |
| 3 | 无敌人 | **MOVE** → 进攻方式目标 | 坐标 / 最近敌人 / 信标 |
| 4 | 已到达目标 | **WAIT** | 原地待命 |

#### 风筝队（kite）

风筝队也支持坐标（`kite_target_x/y`）、自动进攻（`kite_auto_radius`）和冠军信标
（`kite_mode=beacon`）三种目标方式，但不使用 `attack_retreat_radius`，战斗持续到单位
阵亡。每个 Tick 先记录每个敌人的运动轨迹：上一 Tick 的移动方向朝向本单位判为**进攻**，
背离本单位判为**撤退**，垂直于双方连线则单独记为横移；
运动方向、连续 Tick 数、预测下一格和所有候选格都会写入结构化 `tactic_log.jsonl`；
服务器接受计划后，仪表盘战斗日志只追加一条接敌汇总，未接敌的行军 Tick 不占用日志容量。

- 所有单位先评估附近敌人的先锋近战、游侠射程和预测下一 Tick 射程；当前在攻击范围内时优先移动脱离，宁可放弃本次攻击。
- 先锋对先锋在直线一格空隙处提前扫击空格，斜线按敌人历史左右习惯预判；贴脸时先拉开一格再重复预判。
- 游侠对先锋/游侠按进攻或撤退方向使用 `expected_cell` 预判开火；先锋满血追击游侠时保持移动，贴脸后利用 4 HP 对 2 HP 的优势交战。
- 多对一时可用先锋同时走入同一中间格制造移动碰撞，再由游侠攻击敌人原位；重叠的两个先锋在斜线场景分别覆盖左右两个桥接格。
- 风筝队没有人数劣势撤退、回血回城或停止战斗分支；躲避射程属于单 Tick 走位，不会改变编队目标。

#### 游击队（guerrilla）

| 优先级 | 条件 | 动作 | 说明 |
|--------|------|------|------|
| 1 | 该队员当前站在敌方攻击范围内 | **MOVE** 安全走位 | 沿用风筝队的逐 Tick 射程规避；候选格同时评估敌人的预测下一格 |
| 2 | 先锋遇到可提前扫击的先锋 | **SWEEP** | 直线一格空隙预扫；斜线按敌人历史运动习惯选桥接格 |
| 3 | 游侠有合法当前格/预测格射击 | **SHOOT** | 沿用风筝队的移动预判开火 |
| 4 | 有本单位视野内的局部威胁但暂时不宜攻击 | **MOVE** → 最近威胁 | 单兵寻路，不等待或呼叫其他游击单位 |
| 5 | 没有局部威胁 | **MOVE** 固定 8 向漫游 | 每单位按 UUID 分到独立方向；对角用交替四向近似 |

每个游击单位只对本单位**自己视野内**的敌人做风筝决策，默认先锋 4 格 / 游侠 5 格（曼哈顿距离）；`guerrilla_engage_radius`（0=按单位自身视野）可整体调大调小。和风筝队一致，敌方工人和核心可以成为攻击目标，但不属于攻击威胁，不会产生危险区；有先锋或游侠时优先处理战斗威胁。核心只有在周围八格没有敌方先锋/游侠护卫时才会纳入游击目标（落单核心），被护卫的核心会被跳过。

游击队复用风筝队的射程评估、敌人运动预判、先锋预扫和游侠预测射击，但**不读取风筝队的 squad directive、队伍重心或共享目标**；每个单位独立选目标、独立撤离、独立漫游。风筝队仍保留 Nv1 碰撞夹击和斜线协同等组队策略。
...
- 选两者中离 Core 更近的作为目标
- 优先尝试主方向，被挡则尝试其他方向
- 轴向退化兜底：目标与 Core 同行/同列时方向候选只有一个，若该格是墙则追加垂直轴（上/下或左/右）作为侧移候选，防止主方向被墙挡死时静默永久停摆（候选仍受障碍/缰绳/死胡同检查过滤）
- 手动目标 A* 绕行：手动目标模式下主方向格被墙/死胡同时，直接用 A*（预算=距离×60，按目标缓存路径）朝目标真寻路绕行，不再垂直抖动（实测核心在 (-225,-239) 被两面墙挡住东进、连续一轮 0 格并在两格间弹跳）；核心离开缓存路径或新墙挡路时自动重算。自动模式仍走缰绳约束的贪心

**移动失败学习（防卡死循环）：**
- 服务器拒绝 `CORE_DESTINATION_TERRAIN_BLOCKED`（目标格地形阻挡，且视野快照里不是墙）时，把该格一次性记入障碍记忆并持久化，下一 Tick 起自动换方向——否则朝向启发式会每 Tick 重复下发同一步（远程日志观测到 4900+ 条重复告警）。事件坐标是目标格则直接采用，否则用上一 Tick 下发的移动方向推算目标格；无法确定时不猜测
- 收到 `CORE_ALREADY_MOVING` 时自动移动暂停 2 Tick，让位于手动指令/进行中的迁移，避免双驱动互相顶牛
...
- 五个拖拽列：**待命池 / 守家队 / 进攻队 / 风筝队 / 游击队**
- 直接拖动 `V1`、`R2` 等单位芯片换队，无需输入框
- 拖放和参数调整先保留为本地草稿，点击“保存修改”后统一写入 `tactic_config.json`，下个 Tick 生效；“放弃修改”恢复服务器配置
- 下方按守家、进攻、风筝、游击、通用分组显示全部战斗参数，战斗配置不再散落在策略配置面板
- 进攻队和风筝队各自拥有坐标 / 自动进攻 / 冠军信标三选一，只显示当前方式需要的参数；切换方式不会清空其他方式的已有值

**新兵分队比例（`combat_team_ratio`，守:攻:筝:游）：**

- 新生产的游侠 / 先锋不再默认全进守家队，而是按比例把名册向配置靠拢：每次有未编队的新战斗单位，选「当前人数距配置份额缺口最大」的队加入（平局按 守→攻→筝→游 顺序），统计只算存活单位
- 例如现有守家 17 人、其余队 0 人、比例 2:1:1:1，则后续新兵依次补入进攻 → 风筝 → 游击 → 进攻……直到各队人数接近比例
- 留空或填 0（全零）= 关闭比例，恢复旧行为：新兵全部进守家队；格式非法保存时会被拒绝，运行中读到非法值也按关闭处理
- 手动拖拽编队仍然优先：比例只决定“新兵去哪”，不会改动已编队单位
- 配置入口：arena 实时页 → 舰队视图（teams）→ 进攻策略面板「新兵分队比例」；仪表盘战斗分队面板同步提供同一配置项

配置保存到 `tactic_config.json`。`tactic.py` 每 Tick 检查文件变更，保存后无需重启战术进程；
无配置文件或配置损坏时使用内置默认值。策略配置中的“恢复默认”只恢复生产、工人、核心和运行参数，不修改战斗分队。
...
tactic 进程每 Tick 追加发现与解析事件；风筝队仅在计划提交成功且发生接敌时，每 Tick
追加一条全队汇总。dashboard 进程在保存配置时追加配置行；两者用文件锁并发写同一
`battle_log.jsonl`，超过 2MB 自动裁剪保留最新 500 条。筛选选择保存在浏览器 localStorage。
