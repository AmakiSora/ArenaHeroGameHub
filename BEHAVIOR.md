...
**行军受阻自动返家集结**：进攻行军寻路失败（目标超出 A* 扩展预算，或目标向相邻格被墙挡死）时，单位自动切守家队返家逻辑——用**全向贪心步进**（四方向按到核心曼哈顿距离取最优，仍过滤墙/死胡同/满格，不依赖寻路预算）走向核心；到达核心格由被动回血恢复 HP，回到核心附近后目标重新进入寻路预算，进攻自动恢复，无需人工改队伍名册。距核心 ≤2 格时维持原"待命"，不进卸货环添乱。
