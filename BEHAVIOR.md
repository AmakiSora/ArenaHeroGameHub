...
| 优先级 | 条件 | 动作 | 说明 |
|--------|------|------|------|
| 1 | 该队员视野内敌方战斗单位 ≥ 3 | **MOVE** 远离敌群中心 | 撤退，不硬刚 |
| 2 | 该队员视野内敌方战斗单位 = 1 且可攻击 | **SWEEP** / **SHOOT** | 单挑 |
| 3 | 该队员视野内敌方战斗单位 = 1 | **MOVE** → 该敌人 | 主动进攻 |
| 4 | 该队员视野内敌方战斗单位 = 2 且可攻击 | **SWEEP** / **SHOOT** | 可打就打，不追 |
| 5 | 其他 | **MOVE** 固定 8 向漫游 | 每单位按 UUID 分到 N/NE/E/SE/S/SW/W/NW 一路；对角用交替四向近似 |

**各打各的（本地感知）：** 每个游击队员只对本单位**自己视野内**的威胁反应——默认先锋 4 格 / 游侠 5 格（曼哈顿距离，与游戏视野规则一致），`guerrilla_engage_radius`（0=按单位自身视野）可整体调大调小。`turn.visible_enemies` 是**全队视野并集**，若不做本地过滤，一个队员看到的核心会把全队都拉过去；本地过滤后只有真正看见目标的队员去追/撤，其余队员继续各自漫游。

计数只统计**敌方攻击单位**（先锋 / 游侠，不含工人和核心）：敌方核心格上的工人护卫不会把单挑判成 ≥3 而撤退，落单的核心也不会被当成威胁。工人和核心同样不参与撤退质心和单挑目标选择——游击队只追/打/扫威胁，路过工人或核心不纠缠。
