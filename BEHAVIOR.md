...
**自动进攻半径**（`attack_auto_radius`，默认 0，0=不限制）：仅自动进攻（auto 模式）生效，坐标/信标模式不受影响。值 N>0 时，自动目标候选与沿途接敌均被限制在距核心 N 格以内（曼哈顿距离）：目标评分只考虑半径内的目击点（半径内无候选时回退至静态进攻坐标），沿途只追击半径内的可见敌人（半径外敌人直接忽略）。该半径**不约束撤退分支**（`attack_retreat_radius` 遇敌撤退不受影响）。0 表示完全不限制，与旧版行为完全一致。
