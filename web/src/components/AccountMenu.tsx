import { BarChart3, BookOpen, ChevronDown, Gamepad2, GraduationCap, ListOrdered, LogOut } from 'lucide-react'
...
// Slimmed account menu for the dashboard deployment: no API key management
// (the agent credential lives server-side), but sign-out stays — the login
// session is what routes manual commands into the MANUAL plan slot.
export function AccountMenu() {
  const { t } = useTranslation()
  const { user, logout } = useAuth()
  const navigate = useNavigate()
  const [open, setOpen] = useState(false)
  const [dialog, setDialog] = useState<'stats' | null>(null)
  ...
  return <div ref={menuRef} className="relative">
    ...
    {open && <div role="menu" className="panel absolute right-0 top-14 z-50 w-64 rounded-gold-lg p-2 shadow-2xl shadow-black/50">
      <div className="border-b border-white/[.07] px-3 pb-3 pt-2"><p className="text-sm font-medium text-zinc-200">{user?.username}</p></div>
      <nav aria-label="Account navigation" className="mt-1">
        <NavLink role="menuitem" to="/" onClick={() => setOpen(false)} className={({ isActive }) => `flex min-h-11 items-center gap-3 rounded-gold px-3 text-sm transition-colors ${isActive ? 'bg-indigo-deep/55 text-blue-soft' : 'text-zinc-400 hover:bg-white/5 hover:text-zinc-100'}`}><Gamepad2 size={16} />{t('nav.arena')}</NavLink>
        ...
        <button role="menuitem" type="button" onClick={() => { setOpen(false); setDialog('stats') }} className="flex min-h-11 w-full items-center gap-3 rounded-gold px-3 text-sm text-zinc-400 transition-colors hover:bg-white/5 hover:text-zinc-100"><BarChart3 size={16} />{t('nav.stats')}</button>
      </nav>
